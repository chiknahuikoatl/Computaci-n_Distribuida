...
\begin{enumerate}
	...
	Entonces los dos procesos regresan $v$, que es bien 1 o 0, ambos estados finales
	válidos.\\
	Si alguno de los mensajes se pieder, supongamos sin perder generalidad que a $A$
	no le llega el mensaje.\\
	Entonces, $A$ ejecuta la línea 7 y regresa $init_A$.\\
	El proceso $B$ sí recibe el mensaje, por lo que ejecuta la línea 5 y regresa
	$\frac{init_A + 2 (init_B)}{3}$.\\
	Entonces la diferencia entre las dos respuestas es
	$\mid init_A - \frac{init_A + 2 (init_B)}{3} \mid$
	}
\end{enumerate}
\end{document}
