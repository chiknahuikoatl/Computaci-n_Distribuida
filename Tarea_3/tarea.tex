...
%		\begin{algorithmic}[1]
%			\Require $init \in \{0, 1\}$
%			\Function{$\frac{1}{2^k}-AproximateAgreement$}{$init$}
%				\State $send(init)$
...
%		\end{algorithmic}
		...
		\begin{algorithm}
			...
			\Output{$\ v_A^k\  \&\  v_B^k\ $ tales que $\ |v_A^k - v_B^k|\ \leq\ 						\dfrac{1}{2^k}$}
			\KwData{\textbf{Modelo}: $\{A \leftrightarrow B, A \rightarrow B 							\leftarrow B\}$}
			\BlankLine
			$v$ = init \;
			\For{$r = 0$ \KwTo $k -1$}{
				send($v$)\;
